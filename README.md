...
| 偏好     | 根元素状态             | 行为                                   |
| -------- | ---------------------- | -------------------------------------- |
| 亮色     | `<html class="light">` | 固定使用纯白 Glitch 主题，也是默认偏好 |
| 暗色     | `<html class="dark">`  | 固定使用纯黑 Glitch 主题               |
...
### Glitch Editorial token

当前主题采用白纸、墨黑与单一电光品红的 Glitch Blog 编辑视觉系统：
...
正文始终使用黑 / 白高对比组合；`#ff00bc` 不承担长正文颜色，只用于较粗链接、底线、图标和
焦点轮廓。键盘焦点统一使用 2px 品红外框，并尊重 `prefers-reduced-motion` 关闭非必要动效。
