...
- 剪藏同步会从 `vs.do` 分页拉取文章并写入站内 `VS.DO 剪藏` 分类；需要配置 Worker/GitHub secret：`VS_DO_PASSWORD`（或 `VS_DO_TOKEN`）。同步使用稳定 slug（`archive-{source}-{id}`），源站删除不会删除已同步到 TimeAmber 的文章；定时任务默认每 20 分钟同步 10 篇并自动前进到下一页。
