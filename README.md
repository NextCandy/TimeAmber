...
- **首页**：顶栏（首页 / 归档 / 分类 / 关于 / 友链居中，搜索、后台、主题切换为图标）
  → 最新文章两列列表（标题、分类、日期）→ 页脚，服务端直出、无客户端筛选，桌面端收进一屏
- 文章、归档、分类、标签、友链页
...
出现的位置仅限：文章阅读进度条、目录当前项标记、归档年度热力条、正文引用块左边框、
行内代码、首页分类与卡片的 hover 反馈。亮色档必须压到 `L=0.52` 才够 4.5:1，
不要把暗色档那支亮琥珀直接搬过去。新增用途时先在 `:root`/`.dark` 补语义变量、
再在 `@theme inline` 映射，组件里只写类名。

### 文章列表密度

面向读者的文章卡片只呈现标题、分类与发布日期，不加载或显示封面、摘要、阅读时长和
...
一屏，任何让行变高的改动（例如把摘要加回列表）都会连带需要重调那组断点，
