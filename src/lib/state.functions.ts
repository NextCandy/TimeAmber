...
const adminPostFilterInput = z.object({
  ...
  sort: z.enum(["new", "old", "title", "reading"]).optional(),
  visibility: z.enum(["active", "trash"]).optional(),
});

const adminPostsPageInput = adminPostFilterInput.extend({
...
type AdminPostFilters = {
  ...
  dateFrom?: Date;
  dateTo?: Date;
  sort: "new" | "old" | "title" | "reading";
  visibility: "active" | "trash";
};

function parseAdminDate(value: string | undefined, label: string): Date | undefined {
  if (!value) return undefined;
  const parsed = new Date(value.length === 10 ? `${value}T00:00:00.000Z` : value);
  if (Number.isNaN(parsed.getTime())) throw new Error(`${label} 日期无效`);
  return parsed;
}

function normalizeAdminPostFilters(input: z.infer<typeof adminPostFilterInput>): AdminPostFilters {
  const dateTo = parseAdminDate(input.dateTo, "结束");
  if (dateTo && input.dateTo?.length === 10) dateTo.setUTCDate(dateTo.getUTCDate() + 1);
  return {
    query: input.query?.trim() ?? "",
    status: input.status ?? "all",
    category: input.category?.trim() ?? "",
    dateFrom: parseAdminDate(input.dateFrom, "开始"),
    dateTo,
    sort: input.sort ?? "new",
...
function adminPostWhere(sql: ReturnType<typeof postgres>, filters: AdminPostFilters) {
  let where = sql`where true`;
  const add = (clause: ReturnType<typeof sql>) => {
    where = sql`${where} and ${clause}`;
  };
  if (filters.query) {
    ...
    add(sql`(
      ...
      or exists (
        select 1 from public.post_tags search_pt
        join public.tags search_t on search_t.id = search_pt.tag_id
        where search_pt.post_id = p.id and search_t.name ilike ${pattern}
      )
    )`);
  }
  if (filters.status === "published") add(sql`p.published = true`);
  if (filters.status === "draft") add(sql`p.published = false`);
  if (filters.category) add(sql`p.category = ${filters.category}`);
  if (filters.dateFrom) add(sql`coalesce(p.publish_at, p.created_at) >= ${filters.dateFrom}`);
  if (filters.dateTo) add(sql`coalesce(p.publish_at, p.created_at) < ${filters.dateTo}`);
  if (filters.visibility === "trash") add(sql`coalesce(p.listed, true) = false`);
  else add(sql`coalesce(p.listed, true) = true`);
  return where;
}

function adminPostOrder(sql: ReturnType<typeof postgres>, sort: AdminPostFilters["sort"]) {
  if (sort === "title") return sql`p.title asc, p.slug asc`;
  if (sort === "reading") return sql`p.reading_minutes desc, p.slug asc`;
  if (sort === "old") return sql`p.pinned desc, p.created_at asc, p.slug asc`;
  return sql`p.pinned desc, p.created_at desc, p.slug asc`;
}

const adminPostCountCache = new Map<string, { expiresAt: number; total: number }>();

async function cachedAdminPostCount(
  ...
  const cached = adminPostCountCache.get(key);
  if (cached && cached.expiresAt > Date.now()) return cached.total;
  const where = adminPostWhere(sql, filters);
  const [row] = await sql`select count(*)::int as count from public.posts p ${where}`;
  const total = Number(row?.count ?? 0);
  adminPostCountCache.set(key, { expiresAt: Date.now() + 30_000, total });
  return total;
}
...
export const loadAdminPostSlugs = createServerFn({ method: "GET" })
  .validator((value: z.infer<typeof adminPostFilterInput>) => adminPostFilterInput.parse(value))
  .middleware([requireSupabaseAuth])
  .handler(async ({ context, data }): Promise<{ slugs: string[]; truncated: boolean }> => {
    await assertAdmin(context.userId);
    const sql = db();
    const filters = normalizeAdminPostFilters(data);
    const where = adminPostWhere(sql, filters);
    ...
    return { slugs: rows.slice(0, 5000).map((row) => String(row.slug)), truncated };
  });
...
type AdminBatchBucket = { tokens: number; updatedAt: number };
const adminBatchBuckets = new Map<string, AdminBatchBucket>();

/** 服务端兜底令牌桶：前端配置只是体验层，真正的批量请求仍受每管理员 5 req/s 约束。 */
function takeAdminBatchToken(userId: string) {
  const now = Date.now();
  const rate = 5;
  const capacity = 10;
  const previous = adminBatchBuckets.get(userId) ?? { tokens: capacity, updatedAt: now };
  const tokens = Math.min(capacity, previous.tokens + ((now - previous.updatedAt) / 1000) * rate);
  if (tokens < 1) {
    const retryAfterMs = Math.ceil(((1 - tokens) / rate) * 1000);
    throw new Error(`批量操作触发服务端限速，请 ${Math.max(200, retryAfterMs)}ms 后重试`);
  }
  adminBatchBuckets.set(userId, { tokens: tokens - 1, updatedAt: now });
}

const adminBatchInput = z.object({
  slugs: z.array(z.string().trim().min(1).max(300)).min(1).max(5000),
  action: z.enum(["status", "category", "delete", "restore"]),
...
/** 批量写入只接受 slug 白名单，事务返回实际命中项，重复提交保持幂等。 */
export const batchUpdateAdminPosts = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .validator((value: z.infer<typeof adminBatchInput>) => adminBatchInput.parse(value))
  .handler(async ({ data, context }) => {
    await assertAdmin(context.userId);
    takeAdminBatchToken(context.userId);
    const slugs = [...new Set(data.slugs)];
    if (data.action === "status" && !data.status) throw new Error("批量状态操作缺少目标状态");
    if (data.action === "category" && !data.category) throw new Error("批量分类操作缺少目标分类");
    if (data.action === "delete" && data.confirmation !== "DELETE") {
      throw new Error("批量删除需要显式确认 DELETE");
    }

    const sql = db();
    const slugArray = sql.array(slugs);
    const processed = await sql.begin(async (tx) => {
      ...
      if (data.action === "status") {
        return tx`
          update public.posts
          set published = ${data.status === "published"}, updated_at = now()
          where slug = any(${slugArray})
          returning slug
        `;
      }
      if (data.action === "category") {
        return tx`
          update public.posts
          set category = ${data.category!}, updated_at = now()
          where slug = any(${slugArray})
          returning slug
        `;
      }
      if (data.action === "delete") {
        return tx`
          update public.posts
          set listed = false, published = false, updated_at = now()
          where slug = any(${slugArray})
          returning slug
        `;
      }
      return tx`
        update public.posts
        set listed = true, updated_at = now()
        where slug = any(${slugArray})
        returning slug
      `;
    });
    adminPostCountCache.clear();
    const processedSlugs = processed.map((row) => String(row.slug));
    ...
    return {
      ...
      action: data.action,
      processedSlugs,
      skippedSlugs: slugs.filter((slug) => !processedSet.has(slug)),
    };
  });
