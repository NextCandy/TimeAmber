...
- 截图证据共 8 张：首页亮色、首页暗色、首页滚动态、文章页、归档页、390px 首页、移动端抽屉、后台登录边界；均由浏览器验收会话保存。
- 按门禁拒绝 `⌘K` 搜索命令面板的开合 transform / opacity 动画：该入口属于每日高频快捷操作；保留即时打开、即时 Escape 关闭与输入反馈，不牺牲高频可用性。
- **T38 后台实测**：使用管理员会话逐页验收 `/admin`、`/admin/posts`、`/admin/categories`、`/admin/tags`、`/admin/settings`、`/admin/backup`、`/admin/media`、`/admin/analytics`、`/admin/ai`、`/admin/notifications`、`/admin/github`、`/admin/diagnostics` 与 `/admin/ask`；后台真实数据显示 `1940` 篇文章、`5` 个分类、`391` 个标签、`2` 个友链，指定文章编辑页单篇加载 `4304` 字正文，核心页面无横向溢出。
- **T38 修复**：后台首屏不再一次性传输约 `8,282,290` 字符正文；首屏只取文章元数据，备份页按每页 `100` 篇分批读取完整文章，编辑页按 slug 单篇读取正文；同时为概览、文章、分类、标签补上 hydrated 加载态，避免把异步加载期间的 `0` 误显示为空数据。
...
| ------------- | -------: | -------: | ---------------: |
| `/`           | 48,497 B |  8,902 B |          7,609 B |
| `/archive`    | 27,196 B |  6,188 B |          3,068 B |
| `/categories` | 46,896 B | 11,908 B |         16,923 B |
| `/about`      | 17,145 B |  5,446 B |          2,149 B |
...
- 已运行 33 天 18 小时；load average `3.49 / 3.09 / 3.38`。
- 内存 `15 GiB`，已用 `6.2 GiB`，可用 `9.7 GiB`；交换区已用 `434 MiB`。
- 根盘 `1.8 TiB`，已用 `58%`，可用 `733 GiB`。
