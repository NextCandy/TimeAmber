...
import { normalizeTextLineTerminators } from "../../utils/text";
...
export class PostgresAdapter implements IDatabase {
  ...
  async getPublishedPostsPage(limit: number, offset: number): Promise<{ posts: PostSummary[]; total: number }> {
    const safeLimit = Math.max(1, Math.min(limit, 200));
    const safeOffset = Math.max(0, offset);
    const publishedWhere = sql`${pgPosts.published} = true AND (${pgPosts.publishAt} IS NULL OR ${pgPosts.publishAt} <= NOW())`;
    ...
    return {
      ...
      posts: rows.map((post) => ({
        ...
        tags: tagMap.get(post.id) || [],
        pinned: post.pinned,
        publishAt: post.publishAt ? this.ts(post.publishAt) : null,
        seriesSlug: post.seriesSlug || null,
        category: post.category || "",
      })),
    };
  ...
  async getAllPosts(): Promise<(Post & { tags: string[] })[]> {
    ...
    return allPosts.map((post) => ({
      ...
      slug: post.slug,
      title: post.title,
      content: normalizeTextLineTerminators(post.content),
      excerpt: post.excerpt || "",
      coverColor: post.coverColor || "",
      coverImage: post.coverImage || "",
  ...
  async getAdminPostSummariesPage(options: AdminPostListOptions): Promise<AdminPostListPage> {
    ...
    const tagPostIds = tag
      ? (await this.db
        .select({ postId: pgPostTags.postId })
        .from(pgPostTags)
        .innerJoin(pgTags, eq(pgPostTags.tagId, pgTags.id))
        .where(eq(pgTags.name, tag))).map((row) => row.postId)
      : null;
    ...
    const filters = [];
    if (status === "published") filters.push(eq(pgPosts.published, true));
    if (status === "draft") filters.push(eq(pgPosts.published, false));
    if (q) {
      const pattern = `%${q}%`;
      filters.push(or(ilike(pgPosts.title, pattern), ilike(pgPosts.slug, pattern), ilike(pgPosts.excerpt, pattern), ilike(pgPosts.category, pattern))!);
    }
    if (tagPostIds) filters.push(inArray(pgPosts.id, tagPostIds));
    const where = filters.length > 0 ? and(...filters) : undefined;

    const countRows = where
      ? await this.db.select({ count: sql<number>`COUNT(*)::int` }).from(pgPosts).where(where)
      : await this.db.select({ count: sql<number>`COUNT(*)::int` }).from(pgPosts);
    ...
    const rows = where
      ? await this.db.select(selection).from(pgPosts).where(where).orderBy(desc(pgPosts.createdAt)).limit(pageSize).offset(offset)
      : await this.db.select(selection).from(pgPosts).orderBy(desc(pgPosts.createdAt)).limit(pageSize).offset(offset);
  ...
  private async getAdminPostCounts(): Promise<{ all: number; published: number; draft: number }> {
    const rows = await this.db
      .select({ published: pgPosts.published, count: sql<number>`COUNT(*)::int` })
      .from(pgPosts)
      .groupBy(pgPosts.published);
    let published = 0;
    let draft = 0;
    for (const row of rows) {
  ...
  private async getAdminTagCounts(): Promise<{ name: string; count: number }[]> {
    const rows = await this.db
      .select({ name: pgTags.name, count: sql<number>`COUNT(*)::int` })
      .from(pgTags)
      .innerJoin(pgPostTags, eq(pgTags.id, pgPostTags.tagId))
      .groupBy(pgTags.name)
      .orderBy(pgTags.name);
  ...
  async createPost(data: CreatePostInput): Promise<Post> {
    const [newPost] = await this.db
      ...
      .values({
        slug: data.slug,
        title: data.title,
        content: normalizeTextLineTerminators(data.content),
    ...
    return {
      ...
      slug: newPost.slug,
      title: newPost.title,
      content: normalizeTextLineTerminators(newPost.content),
  ...
  async updatePost(slug: string, data: UpdatePostInput): Promise<Post | null> {
    ...
    const [updated] = await this.db
      ...
      .set({
        ...
        ...(data.content !== undefined && { content: normalizeTextLineTerminators(data.content) }),
    ...
    return {
      ...
      slug: updated.slug,
      title: updated.title,
      content: normalizeTextLineTerminators(updated.content),
  ...
  async getAllPages(): Promise<Page[]> {
    ...
    return rows.map((p) => ({
      ...
      slug: p.slug,
      title: p.title,
      content: normalizeTextLineTerminators(p.content),
  ...
  async getPageBySlug(slug: string): Promise<Page | null> {
    ...
    return {
      ...
      slug: page.slug,
      title: page.title,
      content: normalizeTextLineTerminators(page.content),
